...
module.exports = function (server) {
    ...
    var representationSurfInfo = require('../representation/surfInfo');
    ...
    documentation['qualiteAirVille'] = {href: '/qualiteAir/:ville'};
    ...
    server.get(documentation['qualiteAirVille'].href, function (req, res) {
        ...
        http.get(urlQualiteAirVilles, function (response) {
            ...
            response.on('end', function () {
                res.send(200, representationQualiteAirVille.render(req.params.ville, cheerio.load(body)));
            });
        }).on('error', function (e) {
        ...
        http.get(crypto.decrypt(urlSurfInfo), function (response) {
            ...
            response.on('end', function () {
                parseString(body, function (err, result) {
                    res.send(200, representationSurfInfo.render(result));
                });
            });
        }).on('error', function (e) {
            res.send(400, {erreur: e.message});
    ...
    server.get(documentation['meteo'].href, function (req, res) {
        var ville = req.params.ville;
        var urlMeteo = 'http://www.meteociel.fr/previsions/11537/ville.htm';
        ...
        http.get(urlMeteo, function (response) {
            ...
            response.on('end', function () {
                res.send(200, representationMeteo.render(cheerio.load(body)));
            });
        }).on('error', function (e) {
            res.send(400, {erreur: e.message});
        });
